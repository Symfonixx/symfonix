...
<x-admin-layout>
    <x-admin.create-card title="Add New Service" :formUrl="route('admin.services.store')">
            ...
            <div class="col-xl-9 fv-row">
                <select name="service_category_id" class="form-select form-select-solid">
                    ...
                    @foreach($categories as $cat)
                        <option
                            value="{{ $cat->id }}" @selected(old('service_category_id') == $cat->id)>{{ $cat->title }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row mb-8">
            ...
            <div class="col-xl-9 fv-row">
                <input class="form-control" value="{{old('keywords', 'Service,')}}" name="keywords"
                       id="kt_tagify_1"/>
            </div>
        </div>
            ...
            <div class="col-xl-9 fv-row">
                <div class="form-check form-switch form-check-custom form-check-solid me-10">
                    <input class="form-check-input h-30px w-50px"
                           @checked(old('status') == 'published') type="checkbox" name="status"
                           id="flexSwitch30x50"/>
                </div>
            </div>
        </div>
