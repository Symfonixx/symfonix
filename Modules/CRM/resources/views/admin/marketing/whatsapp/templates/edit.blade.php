@section('title', __('crm::whatsapp.pages.edit_template_title'))
...
@section('toolbar')
    ...
    @endphp
    <x-admin.breadcrumb :pageTitle="__('crm::whatsapp.pages.edit_template_title')" :breadcrumbItems="$breadcrumbItems"/>
@endsection
    ...
    <div class="card settings-form-card">
        <div class="card-header border-0 pt-6">
            <div class="card-title d-flex align-items-center gap-3">
                ...
                <div>
                    <h2 class="fw-bold mb-1">{{ $template->displayName() }}</h2>
                    <span class="text-muted fs-7">{{ __('crm::whatsapp.sections.template_form_hint') }}</span>
                </div>
            </div>
        </div>
        <form method="POST" action="{{ route('admin.crm.marketing.whatsapp-templates.update', $template) }}">
            @csrf @method('PUT')
            <div class="card-body pt-2">
                @include('crm::admin.marketing.whatsapp.templates._form', ['template' => $template])
            </div>
